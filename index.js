...
// 1. Mẫu User Mặc định (Main Template) - Đã sửa phần Tuổi
...
NSFW:
  Đặc điểm tình dục:
    ...
    Vai trò: 
    Thói quen:
  Sở thích tình dục (Kinks):
  Giới hạn cấm kỵ:`;

// 1.1 Mẫu NPC Mặc định
...
Mối quan hệ:
  Với nhân vật chính (User): 
  Với nhân vật khác (Char): 
...
// 2. Prompt tạo User Template (Schema)
...
3. **World Consistency**: The fields MUST reflect the specific logic of the provided World Setting.
   - If the world is Xianxia (Tiên Hiệp), include keys like "Căn cốt", "Cảnh giới", "Linh căn".
...
// 2.1 Prompt tạo NPC Template (Schema)
...
2. Structure: YAML keys only. Leave values empty.
3. **World Consistency**: The fields MUST reflect the specific logic of the provided World Setting.
   - If the world is Xianxia (Tiên Hiệp), include keys like "Căn cốt", "Cảnh giới", "Tông môn".
   - If the world is ABO, include "Giới tính thứ hai", "Pheromone".
   - If the world is Cyberpunk, include "Mức độ cơ khí hóa", "Công ty trực thuộc".
...
// 3. User Prompt tạo/nhuận sắc nội dung (Content)
...
[Language]: Please ensure all values are filled in **Vietnamese (Tiếng Việt)** unless specified otherwise.

[Action]:
Output ONLY the YAML data matching the schema.`;

// 4. NPC Prompt tạo nội dung (Content)
...
3. Do not summarize. Do not filter. Fill EVERY field in Vietnamese.
...
const TEXT = {
    PANEL_TITLE: `<span class="pw-title-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></span>Trình tạo Hồ sơ User`,
    ...
    TOAST_WI_FAIL: "Nhân vật hiện tại chưa liên kết World Info, không thể ghi.",
    TOAST_WI_ERROR: "API TavernHelper chưa tải, không thể thao tác World Info.",
    TOAST_SNAPSHOT: "Đã lưu vào lịch sử", 
    TOAST_LOAD_CURRENT: "Đã đọc nội dung hiện tại",
    TOAST_QUOTA_ERROR: "Bộ nhớ trình duyệt đầy (Quota Exceeded), vui lòng xóa bớt lịch sử cũ."
};
...
function getCharacterGreetingsList() {
    ...
    const list = [];
    if (data.first_mes) {
        list.push({ label: "Lời chào đầu #0", content: data.first_mes });
    }
    if (Array.isArray(data.alternate_greetings)) {
        data.alternate_greetings.forEach((greeting, index) => {
            list.push({ label: `Lời chào đầu #${index + 1}`, content: greeting });
        });
    }
    return list;
}
...
function getPresetHintText(val) {
    if (val === 'pure') {
        return "Chế độ thuần khiết (Pure) giúp tránh ảnh hưởng bởi văn phong preset hoặc việc AI tự viết tiếp cốt truyện, nhưng sẽ không có chức năng vượt rào (Jailbreak). Nếu gặp lỗi từ chối trả lời, hãy thử chuyển sang preset khác có Jailbreak.";
    }
    if (val === 'current') {
        return "Sẽ sử dụng preset hiện đang kích hoạt trong Tavern (Main + Jailbreak). Nếu preset hiện tại có chỉ dẫn viết tiếp cốt truyện quá mạnh, nó có thể ảnh hưởng đến kết quả tạo hồ sơ.";
    }
    return `Sẽ bắt buộc sử dụng System Prompt của preset "${val}" để tạo nội dung.`;
}

// ============================================================================
// [Cốt lõi] Logic Tạo (Generation)
// ============================================================================
async function runGeneration(data, apiConfig, isTemplateMode = false) {
    ...
    // [TRANSLATION FIX] Sửa prefill thành tiếng Việt để match với Template
    ...
    const updateDebugView = (messages) => {
        ...
        debugText += `=== Chế độ: ${isTemplateMode ? `${modeStr} Tạo Mẫu (Template)` : (data.mode === 'refine' ? `${modeStr} Nhuận sắc (Refine)` : `${modeStr} Tạo Hồ sơ (Persona)`)} ===\n`;
        debugText += `=== Chiến lược Preset: ${uiStateCache.generationPreset === 'pure' ? '✨ Chế độ thuần khiết (Pure Mode)' : (uiStateCache.generationPreset === 'current' ? 'Theo Tavern Preset (Mặc định)' : uiStateCache.generationPreset)} ===\n\n`;
    ...
    try {
        ...
        const doRequest = async (messages) => {
            if (apiConfig.apiSource === 'independent') {
                ...
                if (window.TavernHelper && typeof window.TavernHelper.generateRaw === 'function') {
                    ...
                    throw new Error("Phiên bản ST quá cũ hoặc chưa cài đặt TavernHelper");
                }
            }
        };
        ...
        try {
            ...
            if (prefillContent && isBadRequest) {
                ...
                toastr.info("API trả về lỗi 400 (có thể do Gemini/Model không hỗ trợ Prefill), đang thử lại chế độ tương thích...");
        ...
        console.error("[PW] Lỗi tạo nội dung:", e);
        throw e;
    } finally { 
    ...
    if (match && match[1]) {
        ...
        if (prefillContent && !responseContent.startsWith(prefillContent) && !responseContent.startsWith("```yaml")) {
            ...
            // [TRANSLATION FIX] Cập nhật logic check tiếng Việt để nối chuỗi
...
function saveHistory(item) {
    ...
    // [TRANSLATION] Sửa check tiêu đề và format tiêu đề sang tiếng Việt
    if (!item.title || item.title === "Chưa đặt tên" || item.title === "未命名") {
        const context = getContext();
        ...
        if (item.data && item.data.type === 'template') {
            ...
            if (mode === 'npc') {
                // Cố gắng tìm tên trong kết quả (hỗ trợ cả tiếng Trung cũ và tiếng Việt mới)
                const nameMatch = item.data.resultText.match(/(?:姓名|Tên):\s*(.*?)(\n|$)/);
...
function generateSmartKeywords(name, content, staticTags = []) {
    ...
    // 1. Thử trích xuất "Biệt danh/Nickname/Alias" từ nội dung
    const aliasMatch = content.match(/(?:别名|昵称|Alias|Biệt danh|Biệt hiệu)[:：]\s*(.*?)(\n|$)/i);
    if (aliasMatch) {
        // Hỗ trợ dấu phẩy tiếng Trung, tiếng Anh, dấu chấm tranh
        const aliases = aliasMatch[1].split(/[,，、]/).map(s => s.trim()).filter(s => s);
        rawKeys.push(...aliases);
    }

    // 2. Tách tên thông minh (Cho tên nước ngoài hoặc tên dịch)
    if (name.includes('·')) {
        // Ví dụ "Sylvia·Pola" -> Thêm "Sylvia"
        rawKeys.push(name.split('·')[0].trim());
    } else if (name.includes(' ')) {
        // Ví dụ "John Doe" -> Thêm "John" (Chặn ký tự đơn)
        const firstName = name.split(' ')[0].trim();
        if (firstName.length > 1) rawKeys.push(firstName);
    }

    // 3. Khử trùng, lọc từ ngắn (<=1 ký tự), bỏ giá trị rỗng
...
async function syncToWorldInfoViaHelper(userName, content) {
    ...
    // Thử đọc tên từ YAML, nếu không có dùng fallback
    const nameMatch = content.match(/(?:姓名|Tên):\s*(.*?)(\n|$)/);
    
    if (isNpc) {
        let npcName = nameMatch ? nameMatch[1].trim() : "";
        if (!npcName) {
            npcName = prompt("Không thể tự động nhận diện tên NPC, vui lòng nhập:", "Người qua đường A");
            if (!npcName) return; 
        }
        entryTitle = `NPC:${npcName}`;
        entryKeys = generateSmartKeywords(npcName, content, ["NPC"]);
    } else {
        // User ưu tiên dùng tên trong YAML, fallback về tên user trong Tavern
...
async function openCreatorPopup() {
    ...
    let presetOptionsHtml = `
        <option value="current" ${uiStateCache.generationPreset === 'current' ? 'selected' : ''}>Theo Tavern Preset (Mặc định)</option>
...
<div class="pw-wrapper">
    <div class="pw-header">
        ...
        <div class="pw-tabs">
            <div class="pw-tab active" data-tab="editor">Hồ sơ</div>
            ...
            <div class="pw-tab" data-tab="history">Lịch sử</div>
        </div>
    </div>
    ...
    <div id="pw-view-editor" class="pw-view active">
        <div class="pw-scroll-area">
                ...
                <div class="pw-template-editor-area" id="pw-template-editor">
                    <div class="pw-template-toolbar">
                        <div class="pw-shortcut-bar">
                            ...
                            <div class="pw-shortcut-btn" data-key="- "><span>List</span><span class="code">-</span></div>
                            <div class="pw-shortcut-btn" data-key="\n"><span>Xuống dòng</span><span class="code">Enter</span></div>
                        </div>
                        <div class="pw-mini-btn" id="pw-reset-template-small" title="Khôi phục mẫu mặc định cho chế độ này" style="margin-left:auto; padding:2px 8px; font-size:0.8em; border:none; background:transparent; opacity:0.6;"><i class="fa-solid fa-rotate-left"></i></div>
                    </div>
            ...
            <div id="pw-result-area" style="display:${activeData.hasResult ? 'block' : 'none'}; margin-top:15px;">
                <div class="pw-relative-container">
                    <textarea id="pw-result-text" class="pw-result-textarea pw-auto-height" placeholder="Kết quả sẽ hiển thị ở đây..." style="min-height: 200px;">${activeData.result}</textarea>
                </div>
                
                <div class="pw-refine-toolbar">
                    <textarea id="pw-refine-input" class="pw-refine-input" placeholder="Nhập ý kiến chỉnh sửa, hoặc bôi đen văn bản ở trên rồi nhấn nút nổi để sửa nhanh..."></textarea>
                    <div class="pw-refine-btn-vertical" id="pw-btn-refine" title="Thực hiện nhuận sắc">
                        <span class="pw-refine-btn-text">Nhuận sắc</span>
        ...
        <div class="pw-footer">
            <div class="pw-footer-group">
                <div class="pw-compact-btn danger" id="pw-clear" title="Xóa trắng"><i class="fa-solid fa-eraser"></i></div>
                <div class="pw-compact-btn" id="pw-copy-persona" title="Sao chép nội dung"><i class="fa-solid fa-copy"></i></div>
                <div class="pw-compact-btn" id="pw-snapshot" title="Lưu vào lịch sử"><i class="fa-solid fa-save"></i></div>
            </div>
            <div class="pw-footer-group" style="flex:1; justify-content:flex-end; gap: 8px;">
                ...
                <button class="pw-btn save" id="pw-btn-apply" style="${isNpc ? 'display:none;' : ''}">Ghi đè hồ sơ hiện tại</button>
            </div>
        </div>
    </div>
    ...
    <div id="pw-diff-overlay" class="pw-diff-container" style="display:none;">
        <div class="pw-diff-tabs-bar">
            <div class="pw-diff-tab active" data-view="diff">
                <div>So sánh thông minh</div><div class="pw-tab-sub">Chọn sửa</div>
            </div>
            <div class="pw-diff-tab" data-view="raw">
                <div>Bản mới (Raw)</div><div class="pw-tab-sub">Xem/Sửa</div>
            </div>
            <div class="pw-diff-tab" data-view="old-raw">
                <div>Bản cũ (Raw)</div><div class="pw-tab-sub">Xem/Sửa</div>
            </div>
        </div>
        ...
        <div class="pw-diff-actions">
            <button class="pw-btn danger" id="pw-diff-cancel">Hủy bỏ</button>
            <button class="pw-btn save" id="pw-diff-confirm">Lưu & Áp dụng</button>
        </div>
    </div>
    ...
    <div id="pw-view-context" class="pw-view">
        <div class="pw-scroll-area">
            ...
            <div class="pw-card-section">
                <div class="pw-row">
                    <label class="pw-section-label">Preset sử dụng để tạo (System Prompt)</label>
            ...
            <div class="pw-card-section">
                <div class="pw-row">
                    <label class="pw-section-label pw-label-gold">Lời chào nhân vật</label>
            ...
            <div class="pw-card-section">
                <div class="pw-row" style="margin-bottom:5px;">
                    <label class="pw-section-label pw-label-blue">World Info (Sách thế giới)</label>
                </div>
    ...
    <div id="pw-view-api" class="pw-view">
        <div class="pw-scroll-area">
            <div class="pw-card-section">
                <div class="pw-row"><label>Nguồn API</label><select id="pw-api-source" class="pw-input" style="flex:1;"><option value="main" ${config.apiSource === 'main' ? 'selected' : ''}>API Chính (Tavern)</option><option value="independent" ${config.apiSource === 'independent' ? 'selected' : ''}>API Độc lập</option></select></div>
        ...
        <div class="pw-scroll-area">
            ...
            <div class="pw-card-section">
                <div class="pw-row" style="margin-bottom:8px; border-bottom:1px solid var(--SmartThemeBorderColor); padding-bottom:5px;">
                    <label style="color: var(--SmartThemeQuoteColor);"><i class="fa-solid fa-circle-info"></i> Phiên bản Plugin</label>
            ...
            <div class="pw-card-section">
                <div class="pw-row">
                    <label style="color: var(--SmartThemeQuoteColor); font-weight:bold;">Giao diện (Theme)</label>
                    <div style="flex:1; display:flex; gap:5px;">
                        ...
                        <button class="pw-btn primary" id="pw-btn-import-theme" title="Nhập file .css" style="padding:6px 10px;"><i class="fa-solid fa-file-import"></i></button>
                        
                        <button class="pw-btn primary" id="pw-btn-download-template" title="Tải mẫu theme" style="padding:6px 10px;"><i class="fa-solid fa-download"></i></button>
                    </div>
                </div>
            </div>
            ...
            <div class="pw-card-section">
                <div class="pw-context-header" id="pw-prompt-header">
                    <span><i class="fa-solid fa-terminal"></i> Xem và Sửa Prompt</span>
                    <i class="fa-solid fa-chevron-down arrow"></i>
                </div>
                <div id="pw-prompt-container" style="display:none; padding-top:10px;">
                    <div class="pw-row" style="margin-bottom:8px;">
                        <label>Mục tiêu sửa</label>
                        <select id="pw-prompt-type" class="pw-input" style="flex:1;">
                            <option value="personaGen">Lệnh tạo/nhuận sắc Hồ sơ User</option>
                            <option value="npcGen">Lệnh tạo/nhuận sắc Hồ sơ NPC</option>
                            <option value="templateGen">Lệnh tạo Mẫu User</option>
                            <option value="npcTemplateGen">Lệnh tạo Mẫu NPC</option>
                        </select>
                    </div>
                    <div class="pw-var-btns">
                        ...
                        <div class="pw-var-btn" data-ins="{{userPersona}}"><span>Hồ sơ User</span><span class="code">{{userPersona}}</span></div>
            ...
            <div id="pw-debug-wrapper" class="pw-card-section" style="display:none; margin-top: 10px; border-top: 1px solid var(--SmartThemeBorderColor); padding-top: 10px;">
                ...
                <div style="font-size: 0.8em; opacity: 0.7; margin-bottom: 5px;">Sau khi bấm "Tạo...", nội dung thực tế gửi cho AI sẽ hiện ở dưới.</div>
        ...
        <div class="pw-scroll-area">
            <div class="pw-history-filters" style="display:flex; gap:5px; margin-bottom:8px;">
                <select id="pw-hist-filter-type" class="pw-input" style="flex:1;">
                    ...
                    <option value="user_persona">Hồ sơ User</option>
                    <option value="npc_persona">Hồ sơ NPC</option>
            ...
            <button id="pw-history-clear-all" class="pw-btn" style="margin-top:15px;">Xóa toàn bộ lịch sử</button>
        </div>
    </div>
</div>
`;

    callPopup(html, 'text', '', { wide: true, large: true, okButton: "Đóng" });
        ...
        if (hasNewVersion) {
            ...
            const html = `
                <div id="pw-new-version-box" style="margin-top:10px; padding:15px; background:rgba(0,0,0,0.2); border: 1px solid var(--SmartThemeQuoteColor); border-radius: 6px;">
                    ...
                    <div id="pw-update-notes" style="font-size:0.9em; margin-bottom:10px; white-space: pre-wrap; color: var(--SmartThemeBodyColor); opacity: 0.9;">${updateInfo.notes || "Không có ghi chú"}</div>
                    <button id="pw-btn-update" class="pw-btn primary" style="width:100%;">Cập nhật ngay</button>
                </div>`;
            $container.html(html);
        } else {
            $container.html(`<div style="margin-top:10px; opacity:0.6; font-size:0.9em;"><i class="fa-solid fa-check"></i> Đang là phiên bản mới nhất</div>`);
        }
    });
    ...
    loadAvailableWorldBooks().then(() => {
        ...
        $('#pw-wi-select').html(`<option value="">-- Thêm World Info tham khảo/đích --</option>${options}`);
    });
...
function bindEvents() {
    ...
    // --- Chuyển đổi chế độ (Kiểu viên thuốc - Dữ liệu cách ly) ---
    $(document).on('click.pw', '.pw-mode-item', function() {
        const mode = $(this).data('mode');
        if (mode === uiStateCache.generationMode) return;
        
        // 1. Lưu dữ liệu hiện tại vào biến ngữ cảnh
    ...
    $(document).on('click.pw', '#pw-btn-update', function() {
        if (!window.TavernHelper || !window.TavernHelper.updateExtension) {
            toastr.error("TavernHelper chưa tải, không thể tự động cập nhật, vui lòng cập nhật thủ công.");
            return;
        }
    ...
    $(document).on('click.pw', '#pw-btn-delete-theme', function() {
        ...
        if (confirm(`Bạn có chắc muốn xóa theme "${current}" không?`)) {
    ...
    $(document).on('change.pw', '#pw-greetings-select', function() {
        ...
        if (idx === "") {
            ...
            $toggleBtn.show().html('<i class="fa-solid fa-angle-up"></i> Mở xem trước');
        }
    });
    ...
    $(document).on('click.pw', '#pw-copy-persona', function() {
        ...
        toastr.success("Đã sao chép thiết lập");
    });
    ...
    $(document).on('click.pw', '#pw-load-main-template', function() {
        if(confirm("Bạn có chắc muốn dùng mẫu User mặc định không? Việc này sẽ ghi đè nội dung soạn thảo hiện tại.")) {
            ...
            toastr.success("Đã tải mẫu User mặc định");
        }
    });
    ...
    // Sự kiện tạo mẫu thông minh (Smart Gen)
    $(document).on('click.pw', '#pw-gen-template-smart', async function() {
        ...
        try {
            ...
            if (!hasCharInfo && !hasWi) {
                const wantGeneric = confirm("Hiện không phát hiện thông tin Thẻ nhân vật hoặc World Info liên quan.\n\nBạn có muốn tạo một mẫu chung chung không?");
                ...
                const useDefault = confirm("Vui lòng chọn nguồn mẫu:\n\nNhấn [OK] để dùng mẫu mặc định có sẵn (Khuyên dùng)\nNhấn [Cancel] để tạo mẫu hoàn toàn mới");
            ...
            if (generatedTemplate) {
                ...
                toastr.success("Tạo mẫu thành công! Vui lòng nhấn 'Lưu mẫu' để xác nhận thay đổi.");
            }
        } catch (e) {
            console.error(e);
    ...
    $(document).on('mousedown.pw', '#pw-float-quote-btn', function (e) {
        ...
        if (selectedText) {
            ...
            if ($input && $input.length) {
                const cur = $input.val();
                const newText = `Sửa đoạn "${selectedText}" thành:`;
    ...
    $(document).on('click.pw', '#pw-btn-refine', async function (e) {
        ...
        if (!refineReq) {
            toastr.warning("Vui lòng nhập ý kiến nhuận sắc");
            isProcessing = false;
            return;
        }
        ...
        try {
            ...
            allKeys.forEach(key => {
                ...
                if (!isChanged) {
                    ...
                    <div class="pw-diff-card new selected single-view" data-val="${encodeURIComponent(valNew)}">
                        <div class="pw-diff-label">Không đổi</div>
                        <textarea class="pw-diff-textarea">${valNew}</textarea>
                    </div>`;
                } else {
                    cardsHtml = `
                    <div class="pw-diff-card old" data-val="${encodeURIComponent(valOld)}">
                        <div class="pw-diff-label">Bản gốc</div>
                        <textarea class="pw-diff-textarea" readonly>${valOld || "(Trống)"}</textarea>
                    </div>
                    <div class="pw-diff-card new selected" data-val="${encodeURIComponent(valNew)}">
                        <div class="pw-diff-label">Bản mới</div>
                        <textarea class="pw-diff-textarea">${valNew || "(Đã xóa)"}</textarea>
                    </div>`;
                }
            ...
            $('.pw-diff-tab[data-view="diff"] .pw-tab-sub').text('Chọn sửa');
            $('.pw-diff-tab[data-view="raw"] div:first-child').text('Bản mới (Raw)');
            $('.pw-diff-tab[data-view="raw"] .pw-tab-sub').text('Xem/Sửa');
            $('.pw-diff-tab[data-view="old-raw"] div:first-child').text('Bản cũ (Raw)');
            $('.pw-diff-tab[data-view="old-raw"] .pw-tab-sub').text('Xem/Sửa');

            if (changeCount === 0 && !responseText) {
                toastr.warning("Nội dung trả về trống, vui lòng chuyển sang 'Xem/Sửa' để kiểm tra");
            } else if (changeCount === 0) {
                toastr.info("Không phát hiện thay đổi nội dung");
            }
            ...
            console.error(e);
            toastr.error("Nhuận sắc thất bại: " + e.message); 
        } finally { 
    ...
    $(document).on('click.pw', '#pw-diff-confirm', function () {
        ...
        saveCurrentState();
        toastr.success("Đã áp dụng sửa đổi");
    });
    ...
    $(document).on('click.pw', '#pw-btn-load-current', function() {
        ...
        if (content) {
            if ($('#pw-result-text').val() && !confirm("Khung kết quả đang có nội dung, bạn có chắc muốn ghi đè không?")) return;
            ...
            toastr.warning("Không tìm thấy mô tả nhân vật hợp lệ");
        }
    });
    ...
    $(document).on('click.pw', '#pw-clear', function () {
        if (confirm("Bạn chắc chắn muốn xóa trắng?")) {
    ...
    $(document).on('click.pw', '#pw-api-fetch', async function (e) {
        ...
        try {
            ...
            toastr.success(`Tìm thấy ${models.length} model`);
    ...
    $(document).on('click.pw', '#pw-reset-prompt', () => {
        if (!confirm("Bạn có chắc muốn khôi phục Prompt mặc định?")) return;
        const type = $('#pw-prompt-type').val();
        if (type === 'templateGen') {
...
function loadThemeCSS(fileName) {
    // [Fix 5] Xóa style tùy chỉnh khi tải file
...
// Áp dụng nội dung CSS tùy chỉnh (dùng cho theme được import)
function applyCustomTheme(cssContent) {
    // [Fix 5] Xóa link file khi dùng custom
...
const renderHistoryList = () => {
    ...
    historyCache.forEach(item => {
        ...
        // [Fix 3] Phân tích định dạng tiêu đề mới
        // NPC: "NPC：Name @ Char"
        // User: "User & Char" hoặc "User模版 (Char)"
        let charName = "";
        if (title.includes(' @ ')) {
    ...
    if (paginated.length === 0) { $list.html('<div style="text-align:center; opacity:0.6; padding:20px;">Chưa có lịch sử</div>'); return; }
    ...
    `);
        $el.on('click', function (e) {
            ...
            if (!$modeBtn.hasClass('active')) {
                $modeBtn.click(); // Trigger click để đổi UI
            }
...
const renderWiBooks = async () => {
    ...
    if (allBooks.length === 0) { 
        container.html('<div style="opacity:0.6; padding:10px; text-align:center;">Nhân vật này chưa liên kết World Info. Vui lòng thêm thủ công trong tab "World Info" hoặc liên kết tại giao diện chính.</div>'); 
        return; 
    }
        ...
        <div class="pw-wi-book">
            <div class="pw-wi-header" style="display:flex; align-items:center;">
                ...
                <div class="pw-wi-header-actions">
                    <div class="pw-wi-filter-toggle" title="Mở/Đóng bộ lọc"><i class="fa-solid fa-filter"></i></div>
                    ${!isBound ? '<i class="fa-solid fa-times remove-book pw-remove-book-icon" title="Gỡ bỏ"></i>' : ''}
                    <i class="fa-solid fa-chevron-down arrow"></i>
                </div>
            </div>
                    ...
                    if (entries.length === 0) {
                        ...
                        <div class="pw-wi-depth-tools">
                            <div class="pw-wi-filter-row">
                                <input type="text" class="pw-keyword-input" id="keyword" placeholder="Tìm từ khóa...">
                            </div>
                            <div class="pw-wi-filter-row">
                                <select id="p-select" class="pw-pos-select">
                                    ...
                                    <option value="before_example_messages">Trước mẫu (Ex)</option>
                                    <option value="after_example_messages">Sau mẫu (Ex)</option>
                                    <option value="at_depth_as_system">@Độ sâu (Sys)</option>
                                    <option value="at_depth_as_assistant">@Độ sâu (Asst)</option>
                                    <option value="at_depth_as_user">@Độ sâu (User)</option>
                                </select>
                                <input type="number" class="pw-depth-input" id="d-min" placeholder="0" title="Độ sâu Min">
                                <span>-</span>
                                <input type="number" class="pw-depth-input" id="d-max" placeholder="Max" title="Độ sâu Max">
                            </div>
                            <div class="pw-wi-filter-row">
                                ...
                                <button class="pw-depth-btn" id="d-reset" title="Khôi phục trạng thái ban đầu của World Info">Đặt lại</button>
                            </div>
                        </div>`);
...
const getPosAbbr = (pos) => {
    ...
    if (pos === 6 || pos === 'at_depth_as_system') return '@Sys'; // Tương thích code cũ
